...
const RunPanel = ({ workspaceId, file }) => {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setHistory((prev) => [
        ...prev,
        { id: entryId, stdin, result: response.data, error: null },
      ]);
    } catch (err) {
      setHistory((prev) => [
        ...prev,
  ...
  return (
    ...
    >
      ...
      {/* Terminal transcript */}
      <div
        className="flex-1 overflow-y-auto px-4 py-3 font-mono text-xs space-y-4"
        style={{
          background: "#F7F8FA",
        ...
        <textarea
          ...
          onFocus={(e) => {
            e.currentTarget.style.boxShadow =
              "inset 0 2px 4px rgba(163,167,178,0.2), 0 0 0 2px #C1652F";
            e.currentTarget.style.borderRadius = "8px";
          }}
          onBlur={(e) => {
            e.currentTarget.style.boxShadow =
              "inset 0 2px 4px rgba(163,167,178,0.1)";
...
const TerminalEntry = ({ entry, fileName }) => {
  ...
  return (
    <div
      ...
      style={{ borderColor: "rgba(163,167,178,0.2)" }}
    >
      ...
      {compileFailed && (
        <div className="mt-1 p-2 rounded-lg" style={pressedStyle}>
          <p className="text-[11px] font-medium" style={{ color: "#C1652F" }}>
            Compilation Error:
          </p>
          <pre
      ...
      {result && !compileFailed && (
        <>
          ...
          {!result.run.stdout && !result.run.stderr && (
            <p className="mt-1 text-[11px]" style={{ color: "#787B85" }}>
              (no output)
            </p>
          )}
          ...
          {result.run.time && (
            ...
            >
              {result.run.time}s
              {result.run.memory
                ? ` · ${Math.round(result.run.memory / 1024)}MB`
                : ""}
            </p>
          )}
        </>
